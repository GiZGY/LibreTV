...
The worker stops at 350 MiB database size, below the provisioned free 0.5 GB allowance. Snapshot creation also checks space before copying. These guards do not guarantee all provider resource limits. Monitor compute usage and workflow runtime separately. Daily refreshes prune only snapshots older than 72 hours, always retaining the latest three published snapshots and a running draft's base. This protects the browser's two-hour pagination pins and rollback. No daily full rescan is scheduled.

Rollback: remove `CATALOG_INDEX_ENABLED` and redeploy to use the existing catalogue path; disable `CATALOG_REFRESH_ENABLED` to stop scheduled writes. Do not delete the database. Three recent snapshots and all snapshots published within 72 hours remain available.
