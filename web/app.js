...
const card=f=>window.OPENSTREAM_LIVE&&window.LiveData&&!LiveData.allowed(f)?'':`<article class="poster-card"><a href="#detail/${f.id}"><div class="poster-art"><img class="poster-ambient" src="${esc(img(f))}" alt="" aria-hidden="true" loading="lazy" referrerpolicy="no-referrer"><img src="${esc(img(f))}" alt="${esc(f.name)}海报" referrerpolicy="no-referrer" loading="lazy" width="500" height="750"><span class="play-hint" aria-hidden="true">▷</span></div><h3 title="${esc(f.name)}">${esc(f.name)}</h3><p class="poster-meta"><span>${esc([f.year||'年份未收录',f.genre||f.type].filter(Boolean).join(' / '))}</span>${Number(f.rating)>0?`<span class="poster-score ${scoreClass(f.rating)}" title="${esc(f.ratingSource||'评分')}">${f.ratingSource==='TMDB'?'<span class="rating-provider">TMDB </span>':''}★ ${esc(f.rating)}</span>`:''}${certificationMarkup(f)}</p></a><button class="quick-favorite favorite-button" aria-label="${favorites.has(f.id)?'取消收藏':'收藏'}${esc(f.name)}" aria-pressed="${favorites.has(f.id)}" data-action="favorite" data-id="${f.id}">${icon('bookmark')}</button></article>`;
const grid=list=>{list=list.filter(f=>!window.OPENSTREAM_LIVE||!window.LiveData||LiveData.allowed(f));return list.length?`<div class="poster-grid">${list.map(card).join('')}</div>`:`<div class="empty"><h2>暂时没有找到影片</h2><p>换个关键词，或试试其他筛选条件。</p><button class="primary" data-action="reset">查看全部影片</button></div>`;};
...
function heroMarkup(){
 ...
 const group=duplicate=>`<div class="film-ribbon-group" ${duplicate?'aria-hidden="true"':''}>${items.map((film,i)=>{
  let markup=card(film).replace('<article class="poster-card"',i>=featured.length?'<article data-loop-copy="true" class="poster-card"':'<article class="poster-card"').replace('loading="lazy"','loading="eager" decoding="async"');
